...
const App = () => {
  ...
  const simplifyMovieItem = (movie) => ({
    ...
    title: movie.title || movie.name || movie.original_name || 'Unknown title',
  ...
  const renderProviderSection = (movie) => {
    ...
    const minHeight = 54;
    if (loading) {
      return <div style={{ minHeight }} className="flex items-center"><p className="text-[11px] text-sky-300">Checking streaming availability...</p></div>;
    }
    if (providers.length === 0) {
      return (
        <div style={{ minHeight }} className="flex items-center">
          <p className="text-[11px] text-red-400">No streaming providers found (region may be unavailable). Click “Free Streaming” again or visit View Details.</p>
        </div>
      );
    }
      ...
      return (
        <div style={{ minHeight }} className="flex flex-col justify-center">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[11px] text-emerald-300">Free available on:</span>
            {freeProviders.map((p) => (
              <span key={p.provider_id} className="bg-emerald-600 text-white px-3 py-1 rounded-lg text-xs font-semibold">
                {p.provider_name}
              </span>
            ))}
          </div>
          {nonFreeProviders.length > 0 && (
            <div className="mt-1">
              <span className="text-[11px] text-yellow-400">Also available (pay): </span>
              <span className="text-[11px] text-yellow-400">
                {nonFreeProviders.map((p, i) => (
                  <span key={p.provider_id}>
                    {p.provider_name}{i < nonFreeProviders.length - 1 ? ', ' : ''}
                  </span>
                ))}
              </span>
            </div>
    ...
    // Only non-free providers
    return (
      <div style={{ minHeight }} className="flex flex-col justify-center">
        <span className="text-[11px] text-red-400 mb-1">Not freely available; available on:</span>
        <div className="flex flex-wrap gap-2">
          {nonFreeProviders.map((p) => (
            <span key={p.provider_id} className="bg-red-800 text-white px-3 py-1 rounded-lg text-xs font-semibold">
              {p.provider_name}
            </span>
          ))}
        </div>
      ...
      <div className="w-full px-4 sm:px-9 pt-16 pb-6">
        <header className="mb-8">
          ...
          <div className="mt-6 w-full px-4 sm:px-6">
            <motion.div
              id="search"
              className={`w-full rounded-[2rem] border border-white/10 bg-slate-950/90 backdropp-blur-xl p-4 shadow-[0_20px_60px_rgba(15,23,42,0.3)] transition duration-500 ${searchHighlight ? 'border-violet-400/60 shadow-[0_0_40px_rgba(139,92,246,0.18)]' : ''}`}
              initial="hidden"
              animate="visible"
